...
**Version:** 0.5.1
**Last updated:** 2026-03-31 by claude-proxy agent (arrow key + menu fix)
**Previous version:** PRD v0.5.0 (2026-03-31, agent 6)
...
- §8: Constraints — added arrow key dual-variant requirement (`\x1b[A` and `\x1bOA`)
- §3.6: Wire format — added note about arrow key variants in input messages
...
Arrow keys use browser names: "Up", "Down", "Left", "Right".
NOTE: Raw terminal escape codes for arrows come in TWO variants:
  CSI mode: \x1b[A \x1b[B \x1b[C \x1b[D  (normal mode)
  SS3 mode: \x1bOA \x1bOB \x1bOC \x1bOD  (application cursor keys)
Any code parsing raw arrow key input MUST check both. PuTTY switches
between them based on terminal mode. See §8 Constraints.
...
| Arrow keys have TWO escape variants | Terminals send either `\x1b[A` (CSI) or `\x1bOA` (SS3) for arrow up, and similarly for down/left/right (`B/C/D`). Both MUST be handled in every arrow key check. PuTTY sends `\x1bOA` in application mode, `\x1b[A` in normal mode. SSH clients switch between them unpredictably. Failure to check both = arrows silently stop working for some users. Applies to all interactive menus: lobby, pickers, restart flow. | claude-proxy commit f0ee5f1 — bug found in production (2026-03-31) |
