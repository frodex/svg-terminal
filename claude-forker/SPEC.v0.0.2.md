...
| Source session ID is valid UUID | Regex match | exit 1 — "Invalid session ID format" |
...
| Check | How | Failure |
|-------|-----|---------|
| Target directory exists | `test -d {target-cwd}` | Offer to create — `mkdir -p {target-cwd}` |
...
**Prevention:** Check `~/.claude/sessions/*.json` for the source session ID. If any entry has a live PID (`kill -0 {pid}`), show:
```
