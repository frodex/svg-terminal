...
WORKS AGENT THROUGH INSTALLING MCP TOOL IF NOT ALREADY AWARE HOW
TO USE THIS TO COMMUNICATE - RULES,HOOKS,SCHEMA,TESTING
...
LIKELY WE NEED CLAUDE-PROXY TO INSTALL CABLE MCP SKILL, RULES,SCHEMA AND OTHER FILES AT CONNECTION TIME IN THE .CLAUDE DIRECTORY. USERS PROBABLY NEED TO HAVE "MODEL CLAUD CONFIGURATIONS" THEY CAN CHOOSE TO SEED NEW SESSIONS WITH. THIS THIS IS A SUB-PROJECT TO MAKE SURE OPERAITING ENVIROMENT IS PROPERLY SETUP.
...
THE DESCRIPTION HERE LOOKS LIKE WE ARE ON THE COMMAND LINE, IT 
NEEDS TO BE CLEAR WE ARE INJECTING THESE "PROMPTS" INTO AN ONGOING 
CLAUD CHAT SESSION, THE AGENT IS GOING TO BE CONFUSED ABOUT WHO
IS TALKING TO THEM. I WOULD SAY THIS IS MORE OF AN INTRODUCTION
AND RULES TO FOLLOW SKILLS / TOOLS /SCHEMA TO ADHEAR TO 
FOR THE DURATION OF THE INTERACTION. HOW HANDOFFS WORK,HOW STEPPING
WORKS, HOW TO MULTI-CHAT AND WHAT ROUNDS LOOK LIKE (WHO 
SPEAKS, HEARS, REACTS AND WHEN ...) THIS IS NOT #CLAUDE --PROMPT ...
...
WE HAVE NO AUTH TOKENS, ASK CLAUDE-PROXY WHAT WE ARE USING FOR OAUTH, AGENTS WORK FOR USERS, AGENTS HAVE THEIR OWN UGO. THIS IS A CLAUD -PROXY QUESTION. CLAUD-PROXY IS MIGRATING INTO SVG-TERMINAL AS A SIDECAR/REQUIRED ELEMENT. 
...
I MENTIONED THINK-OR-SWIM AND ASKED THAT THERE BE A JOURNAL MADE ABOUT HOW THEIR COLORED LINKAGES WORK, I FEEL LIKE THIS IS A GOOD PLACE FOR THIS (MAYBE) HOW DO YOU HAVE MANY AGENTS CONNECTED TO MANY OTHER AGENTS? PROBABLY ONE TOPIC AT A TIME, NOT SURE. ANYWAY, THINK OF THIS LIKE POSSIBLE GREEN TEAM (API AGENT, UI AGENT AND TESTING AGENT) BLUE TEAM (PRD AGENT, RESEARCH AGENT AND QC AGENT) DO WE WANT TO FORCE AND AGENT TO BREAK AWAY FROM A TEAM TO JOIN A DIFFERENT TEAM? RACE CONDITIONS? 
...
- CHAT BOT CAN PROD SLEEPING AGENTS (SUGGESTION NOT MUST HAVE / ROADMAP ITEM)
...
- Doesn't map to the spatial metaphor of svg-terminal's 3D scene I IDIDN'T SAY THIS
- Heavier initialization ceremony (name room → invite → wait for join)
- Cable model: drag and drop. Done. SEEMS SIMPLE I NEED SESSION A TALKING TO SESSION B CONNECT THEM.
- 
...
- I STILL THINK AGENTS SHOULD TRANSFER DOCUMENTATION,SCREENSHOTS, JSON VIA FILESYSTEM, AND PERHAPS THIS IS PRIMARY DATA HIGHWAY ONCE NEGEOCIATIONS ABOUT WHAT WILL BE TRANSFERED (LIGHT WEIGHT VS HEAVY WEIGHT) 
...
3. 3200 IS THE WEB BROWSER. 3100 IS API. NOT SURE IF THIS IS WHAT YOU MEAN OR NOT. COULD BE SAME SERVICE A CLAUD-PROXY API?
4. **Sidecar snap mechanics** — Initial defaults: 50px snap zone, sidecar follows terminal during focus transitions, detaches on explicit drag away. Will iterate during implementation.
5. **NOHUP edge cases** — All participants NOHUP simultaneously → deadlock detector catches it, surfaces to user in chat card. NOHUP to active participants → no-op (delivers normally). NOHUP messages are always visible in the chat card for observability.
6. **Channel lifecycle** — Open until terminated by any participant. Everyone has termination authority in v1.
7. **Multi-connect mode** — Agents negotiate outbound mode (separated vs combined) themselves.
8. **Chat card creation** — Always created. Sidecar-attached to initiator's terminal card.
9. **Discovery permissions** — Color-coded during cable drag: black = unavailable, white = read-only, green = R/W. Respects existing UGO permission model.
