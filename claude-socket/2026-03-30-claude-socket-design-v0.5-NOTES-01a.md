...
THIS SOUNDS MIS-INTREPRETED ONCE CONNECTED THE AGENTS CAN DELIVER MESSAGES USING THESE MODES. ASSUME 3 AGENTS IN CHAT + MODERATOR/USER THIS IS LIKE PM FOR AGENTS. AGENTS MAY WANT TO DIRECT MESSAGES TO ONE AGENT NOT ALL OR A GROUP BUT EXCLUDE AN AGENT. 
...
WHEN AGENTS ARE CONNECTED TO MCP AND BROADCAST THEIR AVAILABILITY THEY SHOULD INCLUDE METADATA, WHAT PROJECTS THEY OWN, PROJECTS THEY WORK ON, SPECALITY RESOURCES, ETC. THEIR RESUME SO TO SAY.**
...
Agents are **persistent Claude Code sessions** managed by claude-proxy. They are not dead processes — they are live sessions sitting at the Claude Code `>` prompt, waiting for input. They already have their MCP tools loaded, their conversation history, their project context. ALTHOUH THE SESSIONS ARE PERSISTENT THE CLAUD AGENTS ARE NOT PERSISTENT THEY SLEEP AFTER EACH TURN AND DO NOT SELF WAKE. TO WAKE AN AGENT THEY MUST BE SENT A PROMPT TO RESPOND TO. CALL AND RESPOND. IT'S POSSIBLE USING A TOOL AGENTS MAY STAY RESIDENT MONITORING THE PROCESS IN AN IDLE STATE BUT FOR LONG PERIODS OF TIME THIS CANNOT BE RELIED ON. AGENTS HAVE A MIND OF THEIR OWN AND MAKING THEM GO THROUGH SOFTWARE GATES OR ATTATCH HOOKS TO THEIR HARNESS IS WISE HERE. SOFTWARE CONFIRMATION THEY ARE COMPLIANT BEFORE ALLOWING TO PASS COULD ALSO BE REQUIRED OF THE CONNECTOR.
...
THIS MESSAGE REQIRES YO TO LOAD THE CLAUDE-CONNECTOR MPC SERVER AND ACTIVATE THE CHAT SKILL/TOOL YOU NEED TO APPLY THESE RULES.MD AND ATTACH THIS RULE.MD TO YOUR SESSION-END-HOOK.JSON <- THESE ARE STUBS, LOOK UP OFFICIAL LOCATIONS, TERMS AND CONFIG FILE CONVENTIONS.
