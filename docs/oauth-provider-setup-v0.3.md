...
- **API Key Model**: Phase 2 replaces cookie-based WS auth with server-side API keys. Keys are issued after cookie authentication, validated on every WS message. Max 10 keys per user, one WS connection per key.
- **Idle Timeout**: API keys expire after 30 minutes of inactivity (`lastActivity` updated on each message, throttled to once per minute)
- **Absolute Timeout**: API keys expire after 24 hours regardless of activity
- **Force Re-login**: Admin action that revokes all API keys for a user. Server sends `reauth-required` WS message, client shows overlay and redirects to login.
- **Admin PIN**: Privileged admin actions (flag changes, deactivate, purge, force re-login, merge) require PIN confirmation. PIN is 4-20 characters, scrypt-hashed. 15-minute sudo window after entry.
- **Rate Limiting** (tiered by endpoint type):
  ...
  - API key issuance: 30/min per IP
- **Dev Mode Hardening**: Requires explicit `AUTH_MODE=dev` env var (missing OAuth vars alone = server refuses to start). Requires `DEV_PASSWORD` env var. Localhost-only by default (`DEV_LOCALHOST_ONLY=0` to opt out).
- **WS Consolidation**: All dashboard communication (session create/restart/fork, layout save) over authenticated WS backchannel. No parallel HTTP for session operations.
