# Partial Screen Fix — Implementation Plan v3 (APPROVED)

> **Status: APPROVED** — reviewed by agent 4 twice (v1 notes + v2 notes). All critical/important issues addressed. Three minor adjustments from v2 review baked in below.
>
> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.
>
> **Supersedes:** `2026-04-02-partial-screen-fix-v2.md`
> **Review trail:** `*-NOTES-01a.md` for both v1 and v2
...
- [ ] **Step 2: Verify current port and test server starts**

```bash
cd /srv/svg-terminal
# Check what port the production server is on
ss -tlnp | grep node | grep -oP ':\K\d+' | head -1
# Check restart script
cat restart-server.sh | grep -i port
...
6. Verify colors render on CP sessions — green prompts, red errors, bold text. Compare visually to a local tmux session to confirm span format is compatible
...
- [ ] **Step 3: Run svg-terminal tests (including E2E)**
