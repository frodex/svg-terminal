...
**Status:** Core UI implemented (2026-04-04): `#top-bar` in `index.html`, styles in `dashboard.css`, wiring + ghost preview + layout reserve + `/auth/me` pill in `dashboard.mjs`. Card headers no longer include ▦ / ⬜. **New session** → `POST /api/sessions/create` (claude-proxy `createSession` via Unix socket, else `tmux new-session`), then `refreshSessions()` + focus new card. Restart / fork still placeholders.
