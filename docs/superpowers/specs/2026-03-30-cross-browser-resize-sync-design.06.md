# Cross-Browser Terminal Resize Sync — Design Spec v06

**Date:** 2026-03-30
**Status:** Ready for implementation planning
**Branch:** camera-only-test
**Preceding:** design.05.md → design.05-NOTES-01a.md → this file
...
1. Added the missing browser-side receive chain — what happens inside the browser when a dimension change arrives (§2.4, new)
...
### 2.4 Browser-side receive chain (what happens when dimension change arrives)

This is the complete path from WebSocket message to card DOM update:

```
Server pushes { type: 'screen', width, height, cursor, lines } via WebSocket (30ms poll)
  │
  ▼
terminal.svg (inside <object>) receives WebSocket message
  → renders new terminal content (text, backgrounds, cursor)
  → calls window._screenCallback(msg) — bridge to dashboard
  │
  ▼
dashboard.mjs _screenCallback fires
  → populates t.screenLines (for copy/paste)
  → calls updateCardForNewSize(t, msg.width, msg.height)
  │
  ▼
updateCardForNewSize(t, newCols, newRows)
  → early return if cols/rows unchanged (no-op)
  → updates t.screenCols, t.screenRows
  → calcCardSize(newCols, newRows) → { cardW, cardH }
  → updates t.baseCardW, t.baseCardH (always, even when focused)
  → *** GUARD: if (t.dom.classList.contains('focused')) return; ***  ← THE BUG
  → updates card DOM: t.dom.style.width/height
  → updates .terminal-inner width/height
```

The guard at the marked line is what blocks focused cards from visually updating when dimensions change from ANY external source (other browser, SSH client, tmux command line). Removing this guard is the core fix.
