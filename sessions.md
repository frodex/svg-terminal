...
[2026-04-08] **URL click area too wide:** When a URL is the last thing on a line, clicking anywhere on the row (including the cursor/input area below) triggers the link. The URL hit detection in `onSceneClick` → `getUrlAtCell` → `screenToCell` needs to bounds-check against the actual URL span width, not the full row.
