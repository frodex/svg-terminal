...
[2026-04-10] **Remote session screen data stops flowing to svg-terminal:** Session works via SSH. Input works via svg-terminal (keystrokes arrive on pippi). Claude-proxy has screen data (getSessionScreen returns content). SSH PTY bridge process alive. Title bar updates flow. But NO screen deltas reach the browser card — shows black. Initial full screen push worked (2 subscribers). Events stopped after that. The TerminalMirror event subscription (cpRegisterTerminal → cpTerminalHandlers) is broken for this session. Next agent: add logging to the terminal event handler in bridgeClaudeProxySession to see if events are being emitted by claude-proxy but not dispatched, or not emitted at all.
[2026-04-10] **REPLACED — Remote session input dies after first keystroke via svg-terminal:** Session works fine via direct SSH/tmux attach. Via svg-terminal, the card shows content on restart but dies after first keystroke. Logs show `capture-pane` failing on local socket — remote sessions have no local socket, only an SSH PTY bridge. The scroll/capture path assumes local tmux, breaks for remote. The PTY write path (pty.write → SSH → tmux) should work but something in the error cascade from the failed capture-pane may be killing the bridge. Needs investigation in claude-proxy's remote session data path — separate the local tmux operations (capture-pane, resize) from the SSH PTY path.
